...
checkpoint = torch.load('checkpoint_0005.pth.tar')
...
gal_embds = np.load('template_emb.npy', allow_pickle= True)
app = Flask(__name__)
@app.route('/predict/shape_matching', methods=['GET', 'POST'])
def shape_matching():
    content = request.json
    print(request.json)
    test_path = content['image_mask']
    write_dir = content['output_dir']
    querry_img_rgb = cv2.imread(test_path)
    querry_img = cv2_to_torch(querry_img_rgb,do_extract = True)
    ...
    sim_dict = {}
    for gal_path, gal_embd in gal_embds.item().items():
        sim_dict[gal_path] = cosine_sim(gal_embd, querry_img_embd)
    ...
    top4_label = sim_dict[-4][0].split('/')[-2].split('.')[0].split('_')[0]
    top5_label = sim_dict[-5][0].split('/')[-2].split('.')[0].split('_')[0]
...
if __name__ == '__main__':
    app.run(host= '10.0.11.69', port = 8872, debug=True)
